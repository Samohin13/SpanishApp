 ...
 *   POST /translate                              — перевод (всем, 300/день, R2-кэш)  [v8]
...
async function handleTranslate(request, env) {
  ...
  // v8: R2-кэш переводов (по примеру TTS). Контент фиксированный (Слово дня у
  // ВСЕХ юзеров одно, примеры в книгах статичны) → одна фраза = ОДИН вызов
  // Gemini на всю аудиторию навсегда. HIT не тратит ни деньги, ни лимит.
  const hashBuf = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(text));
  const hashHex = Array.from(new Uint8Array(hashBuf))
    .map((b) => b.toString(16).padStart(2, "0")).join("");
  const trKey = `translate/v1/${hashHex}.json`;
  if (env.TTS_CACHE) {
    try {
      const cached = await env.TTS_CACHE.get(trKey);
      if (cached) {
    ...
    } catch (_) { /* кэш недоступен — идём в Gemini */ }
  }

  // Лимит проверяем ТОЛЬКО на cache miss.
  ...
  // Кэшируем только успешные ответы (200 с непустым телом).
  if (upstream.status === 200 && respText && env.TTS_CACHE) {
    try {
      await env.TTS_CACHE.put(trKey, respText, {
        httpMetadata: { contentType: "application/json" },
      });
    } catch (_) { /* не смогли записать кэш — не страшно */ }
  }
  ...
  return new Response(respText, {
    ...
    headers: {
      ...
      "Access-Control-Allow-Origin": "*",
      "X-ESPEAK-Cache": "MISS",
      "X-ESPEAK-DailyUsed": String(used + 1),
      "X-ESPEAK-DailyLimit": String(TRANSLATE_DAILY_LIMIT),
    },
  });
}
