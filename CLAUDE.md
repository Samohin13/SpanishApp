...
> Последнее обновление: **2026-04-22, сессия 1 (Flashcards)**
...
- Приложение собирается (`BUILD SUCCESSFUL`), запускается на телефоне
- Главный экран (HomeScreen) отрисовывается
- Экран выбора параметров сессии карточек (FlashcardsSetupScreen) — **работает**, UI ок

**Что сломано и ждёт фикса (в последнем незакоммиченном коде уже исправлено):**
- ⚠️ **База слов пустая** — `DatabaseSeeder.seedIfNeeded()` не вызывался при старте. **FIX:** добавил вызов в `SpanishApp.onCreate()` через `@Inject DatabaseSeeder`. После установки надо **удалить приложение с телефона и поставить заново** (чтобы Application.onCreate сработал с чистой БД). Либо просто Rebuild + Run — `fallbackToDestructiveMigration` пересоздаст БД.
- ⚠️ **Radio-кнопки направления перевода** — клик срабатывал только по кружку. **FIX:** всю «полосу» сделал кликабельной (`Modifier.clickable` на Surface + `onClick = null` у RadioButton).

**Следующий шаг после возвращения:**
1. Пользователь делает `Git → Update Project` + `Rebuild` + удалить-переустановить приложение
2. Проверяет: на экране «Карточки» → «Начать сессию» должна появиться настоящая карточка со словом
3. Если работает — идём в **Коммит 2**: отдельный экран **SettingsVoice** (выбор пола голоса для TTS, сохранение в DataStore, применение в SpanishTts)
...
2. Проверить статус: `git log --oneline -5` и `git status`
3. Проверить, применён ли последний pending-фикс (seeder + clickable) — посмотреть `SpanishApp.onCreate()` и `FlashcardsSetupScreen.DirectionOption`
4. Спросить пользователя: удалось ли поставить приложение, показываются ли карточки
5. По умолчанию следующая задача — **Коммит 2: SettingsVoice**, если не сказано иначе
