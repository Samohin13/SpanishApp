...
- **240** RoadmapLesson в `RoadmapData.kt` — ⚠ roadmap всё ещё на старых 240
  (не показывает 14 V2-only `_5` уроков). Контент V2 для них готов, но не
  отображается в навигации курса. **TODO: добавить 14 RoadmapLesson.**
...
3. **14 V2-only уроков (`_5` суффикс) НЕ в roadmap** — есть в `LessonContentDataV2.kt`,
   ...
   контент если не планируется (~30 мин работы)
