...
**Контент** (все цифры верифицированы grep по коду 2026-05-17):
- 10086 уникальных слов (CleanVocab + 12 ext + JSON-asset + BasicsVocab — все подключены ✓)
- **240/240 уроков** с контентом (16 unit × 15, `LessonContentData.kt` имеет 240 `LessonContent(...)` определений) + auto-generated упражнения (Phase 1-3 движка)
- V2 рефакторинг уроков под новый xlsx-curriculum: 15/240 переписаны (блок A1.1), остальные используют V1 (тоже рабочие)
- 1300+ глаголов спряжения, 159 с полными таблицами
- **100/100 Libros рассказов** (25 A1 + 25 A2 + 15 B1 + 35 B2)
...
2. **V2 курса (xlsx) рефакторинг** — 15/240 уроков переписаны на новый качественный формат; остальные 225 работают на V1 (тоже валидны, но V2 лучше)
